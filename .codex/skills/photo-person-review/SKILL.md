...
batch, run, face, or reference IDs.

## Resume safely

1. Run `uv run ppr status`, `uv run ppr batches`, and
   `uv run ppr target list`. The default catalog is intentionally outside the
   repository; pass `--workspace` only when the user names a different catalog.
   If multiple targets exist and the conversation does not establish the active
   person, ask which target to review before ranking or generating questions.
...
- Translate a named face into face evidence, not a whole-photo decision:
  create or label that person's target and add the face as a positive
  reference. When the user explicitly contrasts another named person with the
  active target, that face may also become an active-target negative reference.
- Record `accept`, `reject`, or `unsure` photo decisions only when the user is
  answering the photo-presence question. Never infer authoritative decisions
...
- Imports are incremental. Re-imports append observations while SHA-256 photo
  records deduplicate content. Persistent face references carry across batches;
